...
- [ANALOG](https://github.com/orangecoloured/analog) - A minimal analytics tool. Tracks events in a span of 10-30 days. `MIT` `Nodejs/Docker` (一款极简的分析工具。追踪10-30天时间段内的事件。MIT许可，基于Node.js/Docker)
- [Aptabase](https://aptabase.com/) - Privacy first and simple analytics for mobile and desktop apps. ([Source Code](https://github.com/aptabase/aptabase)) `AGPL-3.0` `Docker`(面向移动端和桌面端应用的注重隐私且简洁的分析服务。（源代码）AGPL-3.0许可，基于Docker。)
- [AWStats](http://www.awstats.org/) - Generate statistics from web, streaming, ftp or mail server logfiles. ([Demo](https://www.awstats.org/#DEMO), [Source Code](https://github.com/eldy/awstats)) `GPL-3.0` `Perl`(从网站、流媒体、FTP或邮件服务器日志文件中生成统计数据。（演示，源代码）GPL-3.0许可，基于Perl。)
- [Countly Community Edition](https://count.ly) - Real time mobile and web analytics, crash reporting and push notifications platform. ([Source Code](https://github.com/Countly/countly-server)) `AGPL-3.0` `Nodejs/Docker`(实时移动端和网页分析、崩溃报告及推送通知平台。（源代码）AGPL-3.0许可，基于Node.js/Docker。)
- [d8a.tech](https://d8a.tech) - A data collection service that works with your existing Google Analytics setup to capture user activity and send it straight to your own private database. ([Demo](https://lookerstudio.google.com/u/0/reporting/0e4102b6-c38b-4f55-aa25-c1fe91d1c1e9), [Source Code](https://github.com/d8a-tech/d8a)) `MIT` `Go/Docker`(一种数据收集服务，可与现有Google Analytics设置配合使用，捕获用户活动并直接发送到你自己的私有数据库。（演示，源代码）MIT许可，基于Go/Docker。)
- [Daily Stars Explorer](https://emanuelef.github.io/daily-stars-explorer) `⚠` - Track GitHub repo trends with daily star insights to see growth and community interest over time. ([Demo](https://emanuelef.github.io/daily-stars-explorer), [Source Code](https://github.com/emanuelef/daily-stars-explorer)) `MIT` `Go/Nodejs/Docker`(通过每日星标洞察追踪GitHub仓库趋势，查看增长情况和社区兴趣随时间的变化。（演示，源代码）MIT许可，基于Go/Node.js/Docker。)
- [Druid](https://druid.apache.org) - Distributed, column-oriented, real-time analytics data store. ([Source Code](https://github.com/apache/druid)) `Apache-2.0` `Java/Docker`(分布式、列式存储、实时的分析型数据存储。（源代码）Apache-2.0许可，基于Java/Docker。)
- [EDA](https://github.com/jortilles/EDA) - Web application for data analysis and visualization. `AGPL-3.0` `Nodejs/Docker`(用于数据分析和可视化的Web应用程序。AGPL-3.0许可，基于Node.js/Docker。)
- [GoAccess](http://goaccess.io/) - Real-time web log analyzer and interactive viewer that runs in a terminal. ([Source Code](https://github.com/allinurl/goaccess)) `GPL-2.0` `C`(实时网络日志分析器和交互式查看器，可在终端中运行。（源代码）GPL-2.0许可，基于C语言。)
- [GoatCounter](https://www.goatcounter.com) - Easy web statistics without tracking of personal data. ([Source Code](https://github.com/arp242/goatcounter)) `EUPL-1.2` `Go`( 简单的网页统计，不追踪个人数据。（源代码）EUPL-1.2许可，基于Go。)
- [HitKeep](https://hitkeep.com/) - Privacy-first web analytics with goals, funnels, ecommerce tracking, and team management in a single binary with embedded DuckDB (alternative to Google Analytics, Plausible, Umami). ([Source Code](https://github.com/pascalebeier/hitkeep)) `MIT` `Go/Docker`(注重隐私的网页分析，具备目标、漏斗、电商跟踪和团队管理功能，单个二进制文件即可运行，内嵌DuckDB（Google Analytics、Plausible、Umami的替代品）。（源代码）MIT许可，基于Go/Docker。)
- [Litlyx](https://litlyx.com) - All-in-one Analytics Solution. Setup in 30 seconds. Display all your data on an AI-powered dashboard. Fully self-hostable and GDPR compliant. ([Source Code](https://github.com/Litlyx/litlyx)) `Apache-2.0` `Docker`(一体化分析解决方案。30秒内完成设置。在AI驱动的仪表盘上展示所有数据。完全可自行托管，符合GDPR标准。（源代码）Apache-2.0许可，基于Docker。)
- [Liwan](https://liwan.dev/) - Privacy-first web analytics. ([Demo](https://demo.liwan.dev/p/liwan.dev), [Source Code](https://github.com/explodingcamera/liwan)) `Apache-2.0` `Rust/Docker`(注重隐私的网页分析。（演示，源代码）Apache-2.0许可，基于Rust/Docker。)
- [Matomo](https://matomo.org/) - Web analytics that protects your data and your customers' privacy (alternative to Google Analytics). ([Source Code](https://github.com/matomo-org/matomo)) `GPL-3.0` `PHP`(保护数据和客户隐私的网页分析（Google Analytics的替代品）。（源代码）GPL-3.0许可，基于PHP。)
- [Medama Analytics](https://oss.medama.io) - Privacy-first website analytics. Tiny, simple, and cookie-free. ([Demo](https://demo.medama.io), [Source Code](https://github.com/medama-io/medama)) `Apache-2.0/MIT` `Docker/Go`( 注重隐私的网站分析。小巧、简单、无Cookie。（演示，源代码）Apache-2.0/MIT许可，基于Docker/Go。)
- [Metabase](https://metabase.com/) - Easy way for everyone in your company to ask questions and learn from data. ([Source Code](https://github.com/metabase/metabase)) `AGPL-3.0` `Java/Docker`(让公司里的每个人都能轻松提问并从数据中学习。（源代码）AGPL-3.0许可，基于Java/Docker。)
- [Middleware](https://middlewarehq.com/) - Tool designed to help engineering leaders measure and analyze the effectiveness of their teams using the DORA metrics. ([Source Code](https://github.com/middlewarehq/middleware)) `Apache-2.0` `Docker/Python/Nodejs`(旨在帮助工程负责人使用DORA指标衡量和分析其团队效率的工具。（源代码）Apache-2.0许可，基于Docker/Python/Node.js。)
- [Mixpost](https://mixpost.app/) - Social media management software to easily create, schedule, publish, and manage social media content in one place (alternative to Hootsuite and Buffer). ([Source Code](https://github.com/inovector/MixpostApp)) `MIT` `PHP/Docker`(社交媒体管理软件，可在一个地方轻松创建、安排、发布和管理社交媒体内容（Hootsuite和Buffer的替代品）。（源代码）MIT许可，基于PHP/Docker。)
- [Netron](https://netron.app/) - Visualizer for neural network and machine learning models. ([Source Code](https://github.com/lutzroeder/netron)) `MIT` `Python/Nodejs`(神经网络和机器学习模型的可视化工具。（源代码）MIT许可，基于Python/Node.js。)
- [Offen](https://www.offen.dev/) - Fair, lightweight and open web analytics tool. Gain insights while your users have full access to their data. ([Demo](https://www.offen.dev/try-demo/), [Source Code](https://github.com/offen/offen)) `Apache-2.0` `Go/Docker`(公平、轻量级且开源的网页分析工具。在获取洞察的同时，确保用户对其数据拥有完全访问权限。（演示，源代码）Apache-2.0许可，基于Go/Docker。)
